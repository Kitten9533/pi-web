  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 6 }}>
      <div aria-label={t("chat.filesWritten")} style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6 }}>
        {files.map(({ filePath, displayPath, additions = 0, deletions = 0 }) => {
          const name = displayPath || getFileName(filePath);
          ...
          return (
            ...
            >
              ...
              >
                {getFileIcon(getFileName(filePath), 12)}
                <span>{name}</span>
                {additions > 0 && (
        ...
        expanded.patch
          ? (
            <div style={{ border: "1px solid var(--border)", borderRadius: 7, overflow: "hidden" }}>
              <SplitPatchView text={expanded.patch} showFileHeaders />
            </div>
          )
          : (
