...
test("keeps only the last patch when the same file is written then edited", () => {
  const content = [
  ...
  assert.doesNotMatch(files[0]?.patch ?? "", /\+hello/);
});
...
test("counts only the last patch when the same file is written then edited", () => {
  const content = [
    toolCall("1", "write", { file_path: "/abs/out/report.html", content: "hello\n" }),
    toolCall("2", "edit", { path: "/abs/out/report.html" }),
  ];
...
test("keeps only the last edit patch when a file is edited twice in one turn", () => {
  const first = [
    "--- /proj/filecard-test/01-alpha.md",
    "+++ /proj/filecard-test/01-alpha.md",
    "@@ -1 +1 @@",
    "-old",
    "+mid",
  ...
  const content = [
    toolCall("1", "edit", { path: "/proj/filecard-test/01-alpha.md" }),
    toolCall("2", "edit", { path: "/proj/filecard-test/01-alpha.md" }),
  ];
  const files = extractTurnWrittenFiles(content, results(
    okResult("1", { patch: first }),
    okResult("2", { patch: second }),
  ), "/proj");
  assert.equal(files.length, 1);
  assert.match(files[0]?.patch ?? "", /\+new/);
  assert.doesNotMatch(files[0]?.patch ?? "", /\+mid/);
  assert.doesNotMatch(files[0]?.patch ?? "", /-old/);
});

test("rewrites absolute patch headers to paths relative to cwd", () => {
  const patch = [
    "--- /proj/filecard-test/01-alpha.md",
    "+++ /proj/filecard-test/01-alpha.md",
    "@@ -1 +1 @@",
  ...
  const content = [toolCall("1", "edit", { path: "/proj/filecard-test/01-alpha.md" })];
  const files = extractTurnWrittenFiles(content, results(okResult("1", { patch })), "/proj");
  assert.equal(files[0]?.displayPath, "filecard-test/01-alpha.md");
  assert.match(files[0]?.patch ?? "", /^--- a\/filecard-test\/01-alpha\.md$/m);
  assert.match(files[0]?.patch ?? "", /^\+\+\+ b\/filecard-test\/01-alpha\.md$/m);
  assert.doesNotMatch(files[0]?.patch ?? "", /\/proj\//);
});

test("does not rewrite in-hunk ---/+++ content when relativizing headers", () => {
  const patch = [
    "--- /proj/notes.md",
    "+++ /proj/notes.md",
    "@@ -1,2 +1,2 @@",
  ...
  const content = [toolCall("1", "edit", { path: "/proj/notes.md" })];
  const files = extractTurnWrittenFiles(content, results(okResult("1", { patch })), "/proj");
  assert.match(files[0]?.patch ?? "", /^--- a\/notes\.md$/m);
  assert.match(files[0]?.patch ?? "", /^--- old bullet$/m);
  assert.match(files[0]?.patch ?? "", /^\+\+\+ new bullet$/m);
});

test("does not rewrite in-hunk lines that look like absolute file headers", () => {
  const patch = [
    "--- /proj/notes.md",
    "+++ /proj/notes.md",
    "@@ -1,2 +1,2 @@",
  ...
  const files = extractTurnWrittenFiles(content, results(okResult("1", { patch })), "/proj");
  assert.match(files[0]?.patch ?? "", /^--- a\/notes\.md$/m);
  assert.match(files[0]?.patch ?? "", /^--- \/proj\/notes\.md$/m);
  assert.match(files[0]?.patch ?? "", /^\+\+\+ \/proj\/other\.md$/m);
});
