...
export function SplitPatchView({ text, showFileHeaders }: { text: string; showFileHeaders?: boolean }) {
  const { t } = useI18n();
  const files = useMemo(() => parseUnifiedPatch(text), [text]);
  if (!files) return <PatchTextView text={text} />;
  const showHeaders = showFileHeaders ?? files.length > 1;
      ...
      {files.map((file, fileIndex) => (
        ...
        >
          {showHeaders && (
            ...
            >
              <SplitDiffHeader title={formatPatchPath(file.oldPath) || t("i18n.before")} side="left" />
              <SplitDiffHeader title={formatPatchPath(file.newPath) || t("i18n.after")} side="right" />
            </div>
          )}
